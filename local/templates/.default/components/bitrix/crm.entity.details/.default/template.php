...
if($entityTypeID=="1" || $entityTypeID=="2" || $entityTypeID=="3" || $entityTypeID=="4") $button_visible=true;
/*$departments=CIntranetUtils::GetUserDepartments($USER->GetID());

if($USER->GetID()==138) $button_visible=true;
if($USER->GetID()==1) $button_visible=true;
if(in_array(35, $departments)) $button_visible=true;
if(in_array(1653, $departments)) $button_visible=true;
*/
...
?>
<style>
.chat2desk_phones_list
{
		display:none;
}
.chat2desk_phones_list ul
	{
...
.chat2desk_phones_list li:hover
	{
    	color: #3c69c4;;
	}
</style>
<div class="webform-buttons2 " style="padding:5px;">
			<span class="webform-button webform-button-create">
				<span class="webform-button-left"></span>
				<a class="webform-button-text" target="_blank" value="WhatsApp" onclick="debugger; Chat2Desk();"  title="MESSAGER">WHATSAPP</a>
				<span class="webform-button-right"></span>
			</span>
<span class="chat2desk_phones_list" id="chat2desk_phones_list"><ul></ul></span>
</div>
<script>
function Chat2Desk(phone="", transport="")
{
	debugger;
	$('#chat2desk_phones_list').hide();
	BX.ajax.post(
		'/chat2desk/imconnector/ajax.php',
		{
		...
		function (data) 
		{
			...
			if(data!=false)
			{
				...
				if(typeof(top.BXIM)!=='undefined') 
				{
					...
					{
						$('#chat2desk_phones_list').hide();
						top.BXIM.openMessenger(data['dialog'], {RECENT: 'N', MENU: 'N'}); 
					}
					else
					{
						...
						{
							debugger;
							var phones = $('#chat2desk_phones_list ul')[0];
							phones.innerHTML="";
							for (var phone in data['phones']) 
							{
								if(data['phones'][phone].HAS_WHATSAPP=="Y")
								{
									phones.innerHTML = phones.innerHTML+"<li style=\"color:#1058d0;\" data-phone=\""+data['phones'][phone].PHONE+"\" data-transport=\"wa\">"+data['phones'][phone].PHONE+" [whatsapp]</li>";
								}
								else
								{
									phones.innerHTML = phones.innerHTML+"<li data-phone=\""+data['phones'][phone].PHONE+"\" data-transport=\"wa\">"+data['phones'][phone].PHONE+" [whatsapp]</li>";
								}
								/*if(data['phones'][phone].HAS_VIBER=="Y")
								{
									phones.innerHTML = phones.innerHTML+"<li style=\"color:#1058d0;\" data-phone=\""+data['phones'][phone].PHONE+"\" data-transport=\"vi\">"+data['phones'][phone].PHONE+" [viber]</li>";
								}
								else
								{
									phones.innerHTML = phones.innerHTML+"<li data-phone=\""+data['phones'][phone].PHONE+"\" data-transport=\"vi\">"+data['phones'][phone].PHONE+" [viber]</li>";
								}*/


							}
							phones.innerHTML = '<span style="font-size: 10px; color: #c9ced3;text-transform: uppercase;">Select phone number</span>' + phones.innerHTML;


							$('#chat2desk_phones_list ul li').on('click', function() {
								Chat2Desk(this.dataset.phone, this.dataset.transport);
							});
							$('#chat2desk_phones_list').show();

						}
						else
						{
							alert("Can`t find any phone numbers.");
						}
					}
