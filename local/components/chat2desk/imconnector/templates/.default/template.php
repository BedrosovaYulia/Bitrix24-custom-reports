			...
			<div class="imconnector-connect">
				<table class="imconnector-connect-table">
					<?if(!empty($arResult["FORM"]["USER"]["INFO"])):?>
							<tr>
								<td>Configuration1</td>
								<td>
									Connection information :
ACTIVE_STATUS=<?=$arResult["ACTIVE_STATUS"]?>
		...
		<div class="imconnector-intro">
			...
			<?else:?>
				<?if(!empty($arResult["PAGE"])):?>
					...
					?>
					<div class="imconnector-comparawaconnector-public-link-inner-copy-field">
						<span>Токен:</span>
						<input style="width: 250px; padding: 5px;margin: 5px;" id="imconnector-comparawaconnector-public-link-service-token" class="imconnector-comparawaconnector-public-link-inner-copy-field-item" type="text" name="SERVICE_TOKEN" value="<?=htmlspecialcharsbx($arResult['FORM']['SERVICE_TOKEN'])?>">
					</div>
